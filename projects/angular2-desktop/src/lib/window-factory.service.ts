...
export class WindowFactoryService {
  ...
  createWindow<T>(
    ...
    return new Promise((resolve, reject) => {
      let subscription =  this.desktop.wait().subscribe(() => {
        //subscription.unsubscribe();
        let app = this.desktop.applications.find(app => app.id === appId);
        if (app) {
          this.instantiateWindow(appId, params ? params : app.defaultWindowParams)
  ...
  openWindow(id: string, docked?: boolean): void {
    let subscription = this.desktop.wait().subscribe(() => {
      //subscription.unsubscribe();
      let window = this.desktop.getWindow(id);
      let app = this.desktop.applications.find(app => app.id === window.appId);
      if (docked || app.defaultWindowParams.dockPosition) {
        window.dock();
      } else {
  ...
  private instantiateWindow(appId: string, params: WindowParams): Promise<ComponentRef<WindowComponent>> {

    return new Promise<ComponentRef<WindowComponent>>((resolve, reject) => {
      let subscription = this.desktop.wait().subscribe(() => {
